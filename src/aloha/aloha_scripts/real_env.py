...
class RealEnv:
    ...
    def __init__(self, init_node, setup_robots=True):
        ...
        self.puppet_bot_neck = InterbotixManipulatorXS(robot_model="puppet_neck", group_name="arm", gripper_name=None,
                                                       robot_name=f'puppet_left', init_node=init_node)
        if setup_robots:
            self.setup_robots()
        ...
        self.recorder_neck = Recorder('neck', init_node=False)
    ...
    def setup_robots(self):
        ...
        setup_puppet_bot(self.puppet_bot_neck)
    ...
    def get_qpos(self):
        ...
        neck_qpos_raw = self.recorder_neck.qpos
        neck_qpos = neck_qpos_raw[:3]
        return  np.array(neck_qpos)
    ...
    def get_qvel(self):
        ...
        neck_qvel_raw = self.recorder_neck.qvel
        neck_qvel = neck_qvel_raw[:3]
        return np.array(neck_qvel)
    ...
    def get_effort(self):
        ...
        neck_effort_raw = self.recorder_neck.effort
        neck_effort = neck_effort_raw[:3]
        return np.array(neck_effort)
    ...
    def _reset_joints(self):
        ...
        reset_position = START_ARM_POSE[:3]
        move_arms([self.puppet_bot_neck], [reset_position], move_time=1)
    ...
    def step(self, action):
        ...
        self.puppet_bot_neck.arm.set_joint_positions(action[:3], blocking=False)
        time.sleep(DT)
        return dm_env.TimeStep(
...
def test_real_teleop():
    ...
    master_bot_neck = InterbotixManipulatorXS(robot_model="master_neck", group_name="arm", gripper_name=None,
                                              robot_name=f'master_neck',init_node=True)
    setup_master_bot(master_bot_neck)

    # setup the environment
    env = make_real_env(init_node=False) # initial ros node
